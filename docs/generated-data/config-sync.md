...
`ConfigCoordinator.syncConfigSheetRows()` runs the config-sheet floor first and flushes it, then syncs the live Sheet Config/Column Config sheets, without writing TypeScript files. It fails closed unless Spreadsheet Config has a Table with exactly one data row, since the Fill Row IDs endpoint uses that row; the sync reads nothing else from it and never rewrites its cells, apart from Table menu space's data cell, which the config-sheet floor restores to `Not used`. It is reachable from the sheet as a `06_API` endpoint (the run flushes after the action returns). `syncAndFlushConfigSheets()` is that same sync plus an in-orchestrator flush, used from tests. From the terminal, `npm run gen:configs` runs that same floor-then-sync path, then regenerates all three files, so there is no separate sync-only command.
